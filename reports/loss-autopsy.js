export default async function lossAutopsy(events) {
  ...
  const worstTrade = losses.reduce((max, t) => Number(t.grossProfit) < Number(max.grossProfit) ? t : max, losses[0]);
  const worstDate = new Date(Number(worstTrade.time));
  html.push(`<div class="report-body"><h3>Worst Trade Spotlight</h3><p style="color:#94a3b8;font-size:13px;margin-bottom:8px;">The single most damaging trade in the dataset.</p>`);
  ...
  html.push(`<tr style="border-bottom:1px solid #1e293b;"><td style="padding:8px;color:#e2e8f0;"><span class="trade-link" data-position-id="${worstTrade.positionId}">#${worstTrade.positionId}</span></td><td style="padding:8px;color:#94a3b8;">${worstTrade.type}</td><td style="padding:8px;text-align:right;">${worstTrade.volume}</td><td style="padding:8px;text-align:right;">${worstTrade.entryPrice}</td><td style="padding:8px;text-align:right;">${worstTrade.closePrice}</td><td style="padding:8px;text-align:right;">${worstTrade.sl != null ? worstTrade.sl : 'NONE'}</td><td style="padding:8px;text-align:right;color:#ef4444;">${Number(worstTrade.grossProfit).toFixed(1)}</td><td style="padding:8px;text-align:right;color:#94a3b8;">${worstDate.toISOString().slice(5, 10)}</td></tr>`);
